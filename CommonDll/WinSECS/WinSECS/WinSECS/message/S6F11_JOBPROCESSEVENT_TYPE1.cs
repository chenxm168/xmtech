...
namespace WinSECS
{
    public class S6F11_JOBPROCESSEVENT_TYPE1
    {
        public static SECSTransaction makeTransaction(bool isNoPadding , String dataid, String ceid, String rptid, String toolid, String mcmd, String eqst, String bywho, String rptid1, String ipid, String opid, String icid, String ocid, String jobid, String totalgstate, List<S6F11_JOBPROCESSEVENT_TYPE1_GLASS_COUNT> glass_count, String rptid2, String unloadmode, String unloadtype, String splitmode)
        {
			...
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, Encoding.GetEncoding("ks_c_5601-1987").GetBytes(jobid).Length, "JOBID", jobid);
			else
			...
			if(glass_count != null)
			{
				foreach (S6F11_JOBPROCESSEVENT_TYPE1_GLASS_COUNT item in glass_count)
				{
			...
			ListFormat listNode_7 = listNode_6.add(ListFormat.TYPE, 3, "", "") as ListFormat;
			sArray =  unloadmode.Split(' ');
			if (isNoPadding)
				listNode_7.add(Uint1Format.TYPE, sArray.Length, "UNLOADMODE", unloadmode);
			else
				listNode_7.add(Uint1Format.TYPE, 1, "UNLOADMODE", unloadmode);
			if (isNoPadding)
