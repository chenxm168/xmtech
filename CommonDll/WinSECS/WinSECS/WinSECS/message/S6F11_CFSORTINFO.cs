...
namespace WinSECS
{
    public class S6F11_CFSORTINFO
    {
        public static SECSTransaction makeTransaction(bool isNoPadding , String dataid, String ceid, String rptid, String toolid, String mcmd, String eqst, String bywho, String rptid1, List<S6F11_CFSORTINFO_SLOTINFO_COUNT> slotinfo_count)
        {
			...
			ListFormat listNode_SLOTINFO_COUNT = listNode_4.add(ListFormat.TYPE, -1, "SLOTINFO_COUNT", "") as ListFormat;
			if(slotinfo_count != null)
			{
				foreach (S6F11_CFSORTINFO_SLOTINFO_COUNT item in slotinfo_count)
				{
					listNode_SLOTINFO_COUNT.add(item.getMessage(isNoPadding));
				}
			}
