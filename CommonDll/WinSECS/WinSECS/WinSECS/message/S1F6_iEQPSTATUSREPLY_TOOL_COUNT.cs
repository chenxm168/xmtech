...
namespace WinSECS
{
    public class S1F6_iEQPSTATUSREPLY_TOOL_COUNT
    {
        private ListFormat ownerList = new ListFormat();

		private String toolid= "";
		private List<S1F6_iEQPSTATUSREPLY_TOOL_COUNT_EOID_COUNT> eoid_count= new List<S1F6_iEQPSTATUSREPLY_TOOL_COUNT_EOID_COUNT>();

        public S1F6_iEQPSTATUSREPLY_TOOL_COUNT(String toolid, List<S1F6_iEQPSTATUSREPLY_TOOL_COUNT_EOID_COUNT> eoid_count)
        {
			this.toolid = toolid;
        ...
        public ListFormat getMessage(bool isNoPadding)
        {
			...
			if(eoid_count != null)
			{
				foreach (S1F6_iEQPSTATUSREPLY_TOOL_COUNT_EOID_COUNT item in eoid_count)
				{
