...
namespace WinSECS
{
    public class S3F102_CASSETTEINFORMATIONREPLY_TYPE2
    {
        public static SECSTransaction makeTransaction(bool isNoPadding , String ack6, String ptid, String csid, String jobid, String device, String stif, List<S3F102_CASSETTEINFORMATIONREPLY_TYPE2_GLASS_COUNT> glass_count)
        {
			...
			if (isNoPadding)
				listNode_1.add(AsciiFormat.TYPE, Encoding.GetEncoding("ks_c_5601-1987").GetBytes(stif).Length, "STIF", stif);
			else
				listNode_1.add(AsciiFormat.TYPE, 20, "STIF", stif);
			ListFormat listNode_GLASS_COUNT = listNode_0.add(ListFormat.TYPE, -1, "GLASS_COUNT", "") as ListFormat;
			if(glass_count != null)
			{
				foreach (S3F102_CASSETTEINFORMATIONREPLY_TYPE2_GLASS_COUNT item in glass_count)
				{
