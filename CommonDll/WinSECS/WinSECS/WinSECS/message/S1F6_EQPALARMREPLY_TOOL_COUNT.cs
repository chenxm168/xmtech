...
namespace WinSECS
{
    public class S1F6_EQPALARMREPLY_TOOL_COUNT
    {
        private ListFormat ownerList = new ListFormat();

		private String toolid= "";
		private List<S1F6_EQPALARMREPLY_TOOL_COUNT_ALARM_COUNT> alarm_count= new List<S1F6_EQPALARMREPLY_TOOL_COUNT_ALARM_COUNT>();

        public S1F6_EQPALARMREPLY_TOOL_COUNT(String toolid, List<S1F6_EQPALARMREPLY_TOOL_COUNT_ALARM_COUNT> alarm_count)
        {
			this.toolid = toolid;
        ...
        public ListFormat getMessage(bool isNoPadding)
        {
			...
			if(alarm_count != null)
			{
				foreach (S1F6_EQPALARMREPLY_TOOL_COUNT_ALARM_COUNT item in alarm_count)
				{
