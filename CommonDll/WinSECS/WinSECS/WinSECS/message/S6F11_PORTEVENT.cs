...
namespace WinSECS
{
    public class S6F11_PORTEVENT
    {
        public static SECSTransaction makeTransaction(bool isNoPadding , String dataid, String ceid, String rptid, String toolid, String mcmd, String eqst, String bywho, String rptid1, String action, String ptid, String ptmd, String csid, String stif, String csttype, String porttype)
        {
			...
			sArray =  action.Split(' ');
			if (isNoPadding)
				listNode_5.add(Uint1Format.TYPE, sArray.Length, "ACTION", action);
			else
				listNode_5.add(Uint1Format.TYPE, 1, "ACTION", action);
			sArray =  ptid.Split(' ');
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, sArray.Length, "PTID", ptid);
			else
				listNode_5.add(AsciiFormat.TYPE, 2, "PTID", ptid);
			sArray =  ptmd.Split(' ');
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, sArray.Length, "PTMD", ptmd);
			else
				listNode_5.add(AsciiFormat.TYPE, 3, "PTMD", ptmd);
			sArray =  csid.Split(' ');
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, sArray.Length, "CSID", csid);
			else
				listNode_5.add(AsciiFormat.TYPE, 16, "CSID", csid);
			sArray =  stif.Split(' ');
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, sArray.Length, "STIF", stif);
			else
				listNode_5.add(AsciiFormat.TYPE, 20, "STIF", stif);
			sArray =  csttype.Split(' ');
			if (isNoPadding)
				listNode_5.add(Uint1Format.TYPE, sArray.Length, "CSTTYPE", csttype);
			else
				listNode_5.add(Uint1Format.TYPE, 1, "CSTTYPE", csttype);
			sArray =  porttype.Split(' ');
			if (isNoPadding)
				listNode_5.add(AsciiFormat.TYPE, sArray.Length, "PORTTYPE", porttype);
			else
				listNode_5.add(AsciiFormat.TYPE, 6, "PORTTYPE", porttype);
