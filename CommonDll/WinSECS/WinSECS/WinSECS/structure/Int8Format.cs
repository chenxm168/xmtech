...
namespace WinSECS.structure
{
    public class Int8Format : Format
    {
        ...
        public static readonly byte TYPE = 0x18;
        ...
        public override int encoding(int startPos, byte[] bs)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                Array.Copy(ObjectToByte.long2Byte(long.Parse(splits[i])), 0, destinationArray, i * this.DefaultByteLength, this.DefaultByteLength);
            }
        ...
        public override int valueCopy(byte[] bs, int pos)
        {
            ...
            this.Value = ByteToObject.byte2int8(destinationArray);
            return (pos += length);
        }
