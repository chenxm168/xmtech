...
namespace WinSECS.structure
{
    public class Uint8Format : Format
    {
        ...
        public static readonly byte TYPE = 40;
        ...
        public override int encoding(int startPos, byte[] bs)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                Array.Copy(ObjectToByte.long2Byte(long.Parse(splits[i])), 0, destinationArray, i * 8, 8);
            }
        ...
        public override int valueCopy(byte[] bs, int pos)
        {
            ...
            this.Value = ByteToObject.byte2Uint8(destinationArray);
            return (pos += length);
        }
